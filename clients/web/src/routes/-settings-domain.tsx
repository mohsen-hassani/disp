...
// Router-ignored (leading `-`) — see -login.tsx's doc for why. `domain`
// comes in as a prop rather than a `Route.useParams()` read (mirroring
// -note-detail.tsx's `noteId` prop) so tests can mount this directly
// without constructing a param-matching test router.
interface ModuleSettingsPageProps {
  domain: string;
}

export function ModuleSettingsPage({ domain }: ModuleSettingsPageProps): ReactElement {
